    ...
    <AppShell>
      <div className="mx-auto max-w-6xl px-4 sm:px-8 py-8 lg:py-12">
        <div className="min-w-0">
          ...
          <p className="mt-1.5 text-muted-foreground">Here's what's happening in your build world today.</p>
        </div>

        <div className="mt-6 relative">
          <Search className="h-4 w-4 absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Search projects, skills, or buddies..." className="pl-11 h-12 rounded-full bg-surface" />
        </div>

        <Button asChild className="mt-3 w-full sm:w-auto rounded-full">
          <Link to="/explore">Find buddies <ArrowRight className="h-4 w-4" /></Link>
        </Button>
        ...
        <section className="mt-10">
          <div className="flex items-end justify-between mb-4">
            ...
            <Link to="/explore" className="text-sm font-medium text-primary hover:underline">Browse all</Link>
          </div>
            ...
            {recommendedProjects.map((p) => (
              <div key={p.id} className="surface-card p-5 flex flex-col">
                ...
                <Button
                  ...
                  onClick={() => toast.success("✅ Project joined", { description: `You joined ${p.name}` })}
                >
                  Join project
                </Button>
              </div>
              ...
              <div key={p.id} className="surface-card p-5">
                <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{p.members} members · {p.status}</p>
                  </div>
